...
export default function TenantDetails({ id }: TenantDetails) {
    ...
    development: [
      {
        ...
        defaultOpen: true,
        content: (
          <p className={"text-base font-galano mt-4"}>{entity?.description}</p>
        ),
      },
      {
        ...
        content: (
          <>
            <p className={"text-sm font-galano my-4"}>
              ...
              {entity?.location?.division?.name}&nbsp;
              {entity?.location?.division?.state}&nbsp;
              {entity?.location?.division?.country?.longName}
            </p>
            <iframe
              src={`https://maps.google.com/maps?q=${entity?.location?.geo.lat},${entity?.location?.geo.lng}&hl=es;z=14&amp&output=embed`}
              style={{ border: 0 }}
              loading="lazy"
  ...
  return (
    <>
      {entity &&
        config[entity.type.name]?.map((item) => (
          <CollapsibleWrap
            key={item.title}
            title={item.title}
