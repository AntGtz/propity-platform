      ...
      <article className="md:max-w-[264px] w-full flex flex-col min-h-32 font-galano cursor-pointer">
        <header className="w-full relative h-[320px] rounded-md overflow-hidden">
          <Link
            ...
            href={
              withPropertyRedirect
                ? `/propiedad/${entityId}/${propertyId}`
                : `/perfil/${entityId}`
            }
          >
            <Image
              className={"object-cover"}
              src={image ?? "/propertyex.jpg"}
              alt="Property"
              fill
            />
          </Link>
        ...
        <Link
          href={`${withPropertyRedirect ? `/propiedad/${entityId}/${propertyId}` : `/perfil/${entityId}`}`}
          className={"text-sm max-h-16 texte-ellipsis overflow-hidden"}
        >
          {description ??
        ...
        <div className={"flex gap-3 items-center my-1.5"}>
          <Link href={instagram ? `https://instagram.com/${instagram}` : "#"}>
            <svg
              width="22"
              height="22"
              viewBox="0 0 22 22"
          ...
          <Link href={facebook ? `https://facebook.com/${facebook}` : "#"}>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
