...
const testData: AsesorTableData[] = [
  {
    image: "/picClient.jpg",
    name: "Propity MX",
    appointments: "10",
...
export default function Asesores() {
  ...
  const columnHelper = createColumnHelper<AsesorTableData>();
  const columns = [
    ...
    columnHelper.accessor("appointments", {
      header: () => (
        <span className={"font-galano font-normal text-sm"}>Asientos</span>
      ),
      cell: (info) => (
        <span className={"flex flex-col font-galano items-start w-full"}>
          <div className={"font-bold"}>{info.getValue()}</div>
    ...
    columnHelper.accessor("leads", {
      header: () => (
        <span className={"font-galano font-normal text-sm"}>Leads</span>
      ),
      cell: (info) => (
        <span className={"flex flex-col font-galano items-start w-full"}>
          <div className={"font-bold"}>{info.getValue()}</div>
    ...
    columnHelper.accessor("sales", {
      header: () => (
        <span className={"font-galano font-normal text-sm"}>Ventas</span>
      ),
      cell: (info) => (
        <span className={"flex flex-col font-galano items-start w-full"}>
          <div className={"font-bold"}>{info.getValue()}</div>
            ...
            >
              ...
              <div className={"flex gap-2 justify-end"}>
                <svg width="17" height="16" viewBox="0 0 17 16" fill="none">
                  ...
                  />
                  <path
                    d="M10.4399 8C10.4459 8.26634 10.3986 8.53119 10.3008 8.77899C10.203 9.02679 10.0566 9.25256 9.87037 9.44303C9.68411 9.63351 9.46167 9.7848 5 9.88817C8 9.99149 9.29316 10.0447 9.55957 10.0447C9.82598 10.0447 10.0596 9.99149 10.2752 9.88817C10.4908 9.78485 10.6834 9.63351 10.7956 9.44303C10.9078 9.25256 10.9555 9.02679 10.9494 8C10.9494 7.46957 10.7589 6.
                        96086 6.58579C6.21071 6.21071 6 7.46957 6 8H6.44093Z"
                    stroke="black"
                    strokeLinejoin="round"
                  />
                </svg>
    ...
    columnHelper.accessor("leader", {
      header: () => (
        <span className={"font-galano font-normal text-sm"}>Lider</span>
      ),
      cell: (info) => (
        <span className={"flex flex-col font-galano items-start w-full"}>
          <div className={"font-bold"}>{info.getValue().toString()}</div>
        </span>
      ),
    }),
  ];

  const AsesorsTable = withPagination<AsesorTableData>({
    Table: TableWrapped,
  });
